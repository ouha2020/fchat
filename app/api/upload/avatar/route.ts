...
export async function POST(request: Request) {
  ...
  try {
    ...
    const path = `avatars/${member.family_id}/${member.member_id}/${crypto.randomUUID()}.${
    ...
    if (error) throw new ApiRequestError("avatar_upload_failed", 500);

    const { data } = sb.storage.from("chat-images").getPublicUrl(path);
    if (!isSafeHttpUrl(data.publicUrl)) throw new ApiRequestError("avatar_upload_failed", 500);
    return NextResponse.json({ url: data.publicUrl });
  } catch (err) {
    return badRequest(err, "avatar_upload_failed");
  }
}
