...
type MessageType = "text" | "image" | "audio" | "location" | "system";
type AttemptSource = "flush_pending" | "retry_failed";
type PushLogStatus = "sent" | "failed" | "gone" | "skipped";

interface PushBatchStats {
  ok: true;
  scanned: number;
  sent: number;
  gone: number;
...
Deno.serve(async (request) => {
  const authError = requireCronSecret(request, ["PUSH_FLUSH_SECRET"]);
  ...
  try {
    ...
    const result =
      mode === "retry"
        ? await retryFailedPushNotifications(sb)
        : await flushPendingPushNotifications(sb);

    return jsonResponse(result);
  } catch (error) {
    console.warn(
      "[message-push]",
      error instanceof Error ? error.message : "message_push_failed",
    );
    return jsonResponse(
      {
        ok: false,
        error: mode === "retry" ? "push_retry_failed" : "push_flush_failed",
      },
      500,
    );
  }
...
async function flushPendingPushNotifications(
  ...
  const { data, error } = await sb
    ...
    .select(
      "family_id, message_id, member_id, delivery_state, delivered_at, read_at, notified_at, created_at",
    )
  ...
  const plan = await buildCandidatePlan(sb, (data ?? []) as RecipientRow[]);
  return sendCandidatePlan(sb, plan, "flush_pending");
}

async function retryFailedPushNotifications(
  sb: SupabaseClient,
): Promise<PushBatchStats> {
  ...
  const subscriptionIds = unique(
    rows.map((log) => log.subscription_id).filter(isPresent),
  );
  ...
  for (const log of rows) {
    ...
    const sender = message?.sender_member_id
      ? members.get(message.sender_member_id) ?? null
      : null;
    const sub = log.subscription_id
      ? subscriptions.get(log.subscription_id) ?? null
      : null;
    const skipReason = retrySkipReason({
      message,
      recipient,
      member,
      sender,
...
async function buildCandidatePlan(
  ...
  const members = await fetchMembers(sb, unique([...memberIds, ...senderIds]));
  const activeMemberIds = await fetchActiveChatMembers(
    sb,
...
async function sendCandidatePlan(
  ...
  for (const candidate of plan.candidates) {
    ...
    memberSubscriptions.forEach((sub) => {
      const skipReason = subscriptionSkipReason(
        sub,
...
function subscriptionSkipReason(
  ...
  if (messageType === "location" && !sub.location_enabled) {
...
async function fetchRecipientsForMembers(
  ...
  return new Map(
    ((data ?? []) as RecipientRow[]).map((row) => [
      recipientKey(row.message_id, row.member_id),
      row,
    ]),
  );
}
