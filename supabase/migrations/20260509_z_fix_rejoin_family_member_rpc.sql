-- Fix the rejoin RPC after earlier deployments where PostgREST could not
-- resolve the named parameters from its schema cache. Dropping first makes the
-- migration safe even if a previous function was created with different input
-- parameter names; PostgreSQL does not allow renaming input parameters with
